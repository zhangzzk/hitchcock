...
> 一部 **时长任你定的动画短片**——从 60 秒的预告级片段，到几分钟乃至更长的完整 episode 都行（没有硬上限）。风格统一，忠实于原作的逐场节奏，自带叙事性人声对白，全程不需要手改任何中间产物。
...
hitchcock brief answer  -s my-arc --intent "忠实原著、Arcane 画风、约三分钟短片（或者源文撑得起的更长时长）"
...
hitchcock art    generate -s my-arc                   # 默认每个 shot 1 张候选
# 只在某个 shot 想要多张候选时再 `--candidates 2-3`
...
| T2I（主）                   | **gpt-image-2**（OpenAI） |
| T2I（备选）                 | Nano Banana Pro · 豆包 Seedream |
